...
class Synth:
    ...
    def synth_audio(self, text, speaker_id=0, speed=1.0):

        phoneme_ids = self.model.g2p(text)

        text = np.expand_dims(np.array(phoneme_ids, dtype=np.int64), 0)
        text_lengths = np.array([text.shape[1]], dtype=np.int64)
        scales = np.array([0.66667, speed, 0.8], dtype=np.float32)
    ...
    def synth(self, text, oname, speaker_id=0, speed=1.0):

        audio = self.synth_audio(text, speaker_id, speed)
