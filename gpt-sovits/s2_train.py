...
def run(rank, n_gpus, hps):
    ...
    try:  # 如果能加载自动resume
        _, _, _, epoch_str = utils.load_checkpoint(
            utils.latest_checkpoint_path("%s/s2" % hps.data.exp_dir, "D_*.pth"),
            net_d,
            optim_d,
        )  # D多半加载没事
        if rank == 0:
        ...
        _, _, _, epoch_str = utils.load_checkpoint(
            utils.latest_checkpoint_path("%s/s2" % hps.data.exp_dir, "G_*.pth"),
            net_g,
            optim_g,
        )
...
#        net_g.module.load_state_dict(
#            torch.load("pretrained_models/s2G488k.pth", map_location="cpu")["weight"],
#            strict=False,
#        )
        net_d.module.load_state_dict(
            torch.load("pretrained_models/s2D488k.pth", map_location="cpu")["weight"],
            strict=False,
        )
    ...
    for batch_idx, (
        ...
        if global_step % 1000 == 0 and rank == 0:
            utils.save_checkpoint(
                    ...
                    epoch,
                    os.path.join(
                        "%s/s2" % hps.data.exp_dir, "G_{}.pth".format(global_step)
                    ),
            )
            utils.save_checkpoint(
                    net_d,
                    ...
                    epoch,
                    os.path.join(
                        "%s/s2" % hps.data.exp_dir, "D_{}.pth".format(global_step)
                    ),
            )
