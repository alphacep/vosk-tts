...
from text import text_to_sequence, cleaned_text_to_sequence, text_to_sequence_g2p, text_to_sequence_aligned
...
class TextAudioLoader(torch.utils.data.Dataset):
    ...
    def __init__(self, audiopaths_and_text, hparams):
        ...
        self.max_text_len = getattr(hparams, "max_text_len", 350)
...
class TextAudioSpeakerLoader(torch.utils.data.Dataset):
    ...
    def __init__(self, audiopaths_sid_text, hparams):
        ...
        self.cleaned_text = getattr(hparams, "cleaned_text", False)
        self.g2p_text = getattr(hparams, "g2p_text", False)
        self.aligned_text = getattr(hparams, "aligned_text", False)
    ...
    def _filter(self):
        ...
        for audiopath, sid, text, cleaned in self.audiopaths_sid_text:
            ...
            '''
            if self.min_text_len <= len(text) and len(text) <= self.max_text_len:
                audiopaths_sid_text_new.append([audiopath, sid, text, cleaned])
                lengths.append(os.path.getsize(audiopath) // (2 * self.hop_length)) # !
                '''
    ...
    def get_audio_text_speaker_pair(self, audiopath_sid_text):
        ...
        audiopath, sid, text, cleaned_text = audiopath_sid_text[0], audiopath_sid_text[1], audiopath_sid_text[2], audiopath_sid_text[3]
        text = self.get_text(text, cleaned_text)
        spec, wav = self.get_audio(audiopath)
    ...
    def get_text(self, text, cleaned_text):
        if self.aligned_text:
            text_norm = text_to_sequence_aligned(cleaned_text)
        elif self.g2p_text:
            text_norm = text_to_sequence_g2p(text)
        elif self.cleaned_text:
            text_norm = cleaned_text_to_sequence(text)
